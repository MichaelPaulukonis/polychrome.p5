import defaults from '@/assets/default.settings.json'
import { QuickSettings } from '~/src/gui/quicksettings'
import {
  getRandomColors,
  drawModes
} from '~/src/params'
...
const setupGui = ({ p5, sketch, params, fillParams, outlineParams, handleResize }) => {
  ...
  const zoneGui = QuickSettings.create(p5.windowWidth - 440, 180, 'Zonal Painting', pDoc)
    ...
    .addButton('Commit Zone', sketch.commitZone)
    .addButton('Flip Zone H', () => sketch.flipZone({ axis: sketch.HORIZONTAL }))
    .addButton('Flip Zone V', () => sketch.flipZone({ axis: sketch.VERTICAL }))
    .addButton('Mirror Zone H', () => sketch.mirrorZone({ axis: sketch.HORIZONTAL }))
