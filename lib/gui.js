...
gui.addColor(params, 'color').listen()
...
olGui.addColor(params.outline, 'color').listen()
olGui.add(params.outline, 'transparent').listen()
olGui.add(params.outline, 'transparency').min(0).max(100).step(1).listen()

var c = selectToRadios(gui.add(params, 'scheme', schemeList))
c.__radios.map(colorLabel)

var oc = selectToRadios(olGui.add(params.outline, 'scheme', schemeList))
