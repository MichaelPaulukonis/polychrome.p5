...
function drawGrid (xPos, yPos) {
  ...
  // UGH - would prefer to NOt copy everything......
  // we can make the pieces functions, and set the functions
  // allows for more mix-matching...
  // since I also want to originate in the vrious corners
  // TODO: also the size should/could be inverted
  if (!params.invert) {
    // original upper-left
    for (var gridY = 0; gridY < height; gridY += stepX) {
      for (var gridX = 0; gridX < width; gridX += stepX) {
        drawstuff(gridX, gridY, stepX)
      }
    }
  } else {
    for (var gridY = height; gridY > 0; gridY -= stepX) {
      for (var gridX = width; gridX > 0; gridX -= stepX) {
        drawstuff(gridX, gridY, stepX)
      }
    }
  }
