...
function drawGrid (xPos, yPos) {
  ...
  // eh, this is a preliminary algorithm
  const sw = params.useOutline ? (gridParams.step / 5) : 0
  strokeWeight(sw)
  ...
  function paintActions (gridX, gridY, step) {
    setpaintMode(gridX, gridY, params)
    setStrokeMode(gridX, gridY, params.outline)
    push()
...
// let curCycle = 0
function setpaintMode (gridX, gridY, params) {
  ...
  switch (mode) {
    case 1:
      fill(width - gridX, gridY, 100, transparency)

      break

    case 2:
      fill(gridX / 2, gridY / 2, 100, transparency)
      break

    case 3: // offset from default
      var x = (gridX + width / 2) % width
      var y = (height - gridY + height / 2) % height
      fill(x, y, 100, transparency)
      break
    ...
    case 7:
      {
        const grayScaled = (gridY * 255) / height
        fill(255 - grayScaled, transparency)
      }
      break
    ...
    case 8:
      {
        ...
        // I do not like this global var
        // also, this should be conditional? Or not. I prefer it this way....
        if (donePainting) {
          params.curCycle = (params.curCycle + 1) % colors.length
          donePainting = false
        }
        fill(colorAlpha(colors[params.curCycle], transparency))
      }
      break

    case 9:
      fill(colorAlpha(params.color, transparency))
      break

    case 0:
    default:
      fill(gridX, height - gridY, 100, transparency)
      // const sca = colorAlpha('#FFF', transparency)
      // stroke(sca)
      stroke(gridX, height - gridY, 100, transparency)
      break
  }
}

function setStrokeMode (gridX, gridY, params) {
  // TODO: I don't understand the third-parameter here, in HSB mode.
  const transparency = params.transparent ? parseInt(params.transparency, 10) / 100 : 100
  var mode = parseInt(params.paintMode, 10)
  switch (mode) {
    case 1:
      stroke(width - gridX, gridY, 100, transparency)

      break

    case 2:
      stroke(gridX / 2, gridY / 2, 100, transparency)
      break

    case 3: // offset from default
      var x = (gridX + width / 2) % width
      var y = (height - gridY + height / 2) % height
      stroke(x, y, 100, transparency)
      break
    ...
    case 6:
      {
        const grayScaled = (gridY * 255) / height
        stroke(grayScaled, transparency)
      }
      break
    ...
    case 7:
      {
        const grayScaled = (gridY * 255) / height
        stroke(255 - grayScaled, transparency)
      }
      break
    ...
    case 8:
      {
        ...
        stroke(colorAlpha(colors[params.curCycle], transparency))
      }
      break

    case 9:
      stroke(colorAlpha(params.color, transparency))
      break
