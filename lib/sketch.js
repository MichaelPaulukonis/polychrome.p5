...
function setup () {
  ...
  // colorMode(HSB, 360, 100, 100, 1) // default
...
function drawGrid (xPos, yPos) {
  ...
  const sw = params.outline ? (gridParams.step / 5) : 0
  strokeWeight(sw)
  ...
  function drawstuff (gridX, gridY, step) {
    ...
    stroke(255) // should be a setting, I guess
    push()
...
function setpaintMode (gridX, gridY) {
  ...
  switch (mode) {
    case 1:
      ...
      // let c = color()
      // fill(color(`hsba(${width - gridX}, ${gridY}, ${transparency}, ${transparency})`))
    ...
    case 8:
      {
        ...
        fill(colorAlpha(colors[curCycle], transparency))
      }
      break

    case 9:
      fill(colorAlpha(params.color0, transparency))
      break
