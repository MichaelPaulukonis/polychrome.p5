...
function flip (axis) {
  var tmp = createImage(width, height)
  loadPixels()
  tmp.pixels = pixels
  // ugh. tmp.pixels is set
  // but never writes when used with image()
  // neither here nor console
  push()
  if (axis === HORIZONTAL) {
    // scale(-1)
    // image(img, width / 2, 0)
    // image(tempImage, width / 2, height / 2)
    ...
    /** END WORKS */
    // based on https://forum.processing.org/two/discussion/22546/how-do-i-flip-video-in-canvas-horizontally-in-p5js
    translate(0, height)
    scale(1, -1)
    image(tempImage, 0, 0, width, height)
  } else {
    // scale(1)
    // image(tmp, 0, height / 2)
    translate(width, 0)
    scale(-1, 1)
    image(tempImage, 0, 0, width, height)
  }
