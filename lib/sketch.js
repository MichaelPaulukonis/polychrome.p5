...
function drawTest (xPos, yPos, params) {
  // var rows = 10
  // var cols = 10

  // eh. I'd _like_ to have it approach _1_
  // need to use _map_
  // var rows = (height - xPos) / 10
  // var cols = (width - yPos) / 10
  ...
  // this needs to be logarithmically scaled, somehow...
  // https://stackoverflow.com/a/846249/41153
  // https://www.npmjs.com/package/log-scale
  const tSize = map(-rows, -50, -1, 4, width, true)
  console.log(`rows: ${rows} cols: ${cols} size: ${tSize}`)
  textSize(tSize) // the fewer rows, the larger....

  // background(32)
  const getchar = params.nextCharMode === '1' ? t.getchar : t.getcharRandom
  ...
  for (var y = 0; y < rows; y++) {
    for (var x = 0; x < cols; x++) {
      ...
      setPaintMode(pixelX, pixelY, params.fill, fill)
      setPaintMode(pixelX, pixelY, params.outline, stroke)

      text(getchar(), pixelX, pixelY)
    }
  }
}
...
function drawCircle (xPos, yPos, params) {
  ...
  while (arclength < circumference) {
    ...
    if (params.cumulativeRotation) {
      ...
      setPaintMode(xPos, yPos, params.fill, fill)
      setPaintMode(xPos, yPos, params.outline, stroke)
