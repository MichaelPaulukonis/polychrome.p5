...
let sketch = (p) => {
  ...
  p.mousePressed = () => {
    ...
    if (params.fadeBackground) {
      p.push()
      p.fill(p.colorAlpha('#FFFFFF', 0.5))
      p.noStroke()
      ...
      case 8:
        {
          ...
          func(p.colorAlpha(colors[params[`${prefix}_curCycle`]], transparency))
        }
        break

      case 9:
        func(p.colorAlpha(params[`${prefix}_color`], transparency))
        break
  ...
  function save_sketch () {
    ...
    const pad = function (nbr, width, fill) {
      fill = fill || '0'
      nbr = nbr + ''
      return nbr.length >= p.width ? nbr : new Array(p.width - nbr.length + 1).join(fill) + nbr
    }
